...
function ThungRacPhieuNhap() {
    ...
    // ... (Giữ nguyên các hàm Khoi_Phuc, xoa_tatca, xoa_phieunhap_theo_id)
    ...
    const currentItems = DanhSachPhieuNhap.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(DanhSachPhieuNhap.length / itemsPerPage);

    const paginate = (pageNumber) => setCurrentPage(pageNumber);

    // Xử lý khi xóa item ở trang cuối cùng và trang đó trở nên rỗng
    useEffect(() => {
    ...
    return (
        <div className="w-full min-h-screen bg-[#fcfcfd] text-slate-900 font-sans z-0 relative pb-10">
            {
                errHT && (
            ...
            <div className="p-8 space-y-4">
                {currentItems && currentItems.length > 0 ? (
                    <>
                        {/* Map qua currentItems thay vì DanhSachPhieuNhap */}
                        ...
                        {totalPages > 1 && (
                            <div className="flex justify-center items-center pt-6 pb-2 gap-2">
                                <button 
                                    ...
                                    className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold transition-all ${currentPage === 1 ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-indigo-600 shadow-sm'}`}
                                >
                                ...
                                {Array.from({ length: totalPages }, (_, i) => i + 1).map((number) => (
                                    <button
                                        ...
                                        className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-black transition-all ${currentPage === number ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200 scale-105' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 shadow-sm'}`}
                                    >
                                        {number}
                                    </button>
                                ))}
                                ...
                                <button 
                                    ...
                                    className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold transition-all ${currentPage === totalPages ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-indigo-600 shadow-sm'}`}
                                >
