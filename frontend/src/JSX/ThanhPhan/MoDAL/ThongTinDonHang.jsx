...
const ThongTinDonHang = ({ DuLieu }) => {
  ...
  useEffect(() => {
    const fetchPhi = async () => {
      try {
        const diaChiGiao = ThongTinDatDon?.ThongTin_KhachHang?.DiaChi_GiaoHang;
        if (diaChiGiao) {
          const res = await API.CallAPI(undefined, { url: `/NguoiDung/PhiGiaoHang?DiaChi=${diaChiGiao}`, PhuongThuc: 2 });
          if (res.ThanhCong) {
            setPhiVanChuyen(res.PhiShip);
          } else {
            setPhiVanChuyen(-1);
            ThongBao.ThongBao_CanhBao(res.message);
          }
        }
      } catch (error) {
        console.error('Lỗi phí ship:', error);
      }
    };
    fetchPhi();
  ...
  const Luu_DiaChi = async () => {
    ...
    setloading(true);
    try {
      const form = fun.objectToFormData({ DiaChi: DiaChi, IDND: ThongTinNguoiDung?.IDND });
      const response = await API.CallAPI(form, { url: '/NguoiDung/ChinhSuaDiaChi', PhuongThuc: 1 });
      if (response.ThanhCong) {
        ThongBao.ThongBao_ThanhCong(response.message);
        setreload(!reload);
        setinput_diachi(false);
      } else {
        ...
        <main className="flex-1 overflow-y-auto p-4 space-y-5 pb-32">
          ...
          {input_diachi && (
            <section className="bg-gradient-to-r from-yellow-50 to-yellow-100 p-4 rounded-2xl border border-yellow-200 text-center text-yellow-500">
              ...
              <input type="text" onChange={(e) => setDiaChi(e.target.value)} placeholder="Nhập địa chỉ nhận hàng" className="mt-2 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-full" />
                      ...
                      <div className="flex justify-between items-end mt-1">
                        ...
                        {DuLieu?.TrangThai === 2 ? (
                          <div className="flex items-center bg-slate-100 rounded-xl p-1 border border-slate-200 shadow-inner">
                            <button onClick={() => { GiamSoLuong(index) }} className="w-7 h-7 flex items-center justify-center text-slate-400">-</button>
                            <span className="w-8 text-center text-xs font-black text-slate-700">{item.SOLUONG}</span>
                            <button onClick={() => { TangSoLuong(index) }} className="w-7 h-7 flex items-center justify-center bg-blue-600 text-white rounded-lg shadow-lg shadow-blue-200">+</button>
                          </div>
                        ) : (
          ...
          <section onClick={toggleModal} className="flex items-center justify-between p-4 bg-orange-50 border border-orange-100 rounded-2xl cursor-pointer active:scale-95 transition-transform">
          ...
          <section className="space-y-2 pt-4 border-t">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-green-500">-{fun.formatCurrency(GiaTriGiamVoucher)}</span>
            </div>
          </section>
        </main>

        <footer className="bg-white sticky bottom-0 z-10 p-4 border-t">
          {DuLieu.TrangThai === 2 && (
            <div className="bg-red-50 border-b border-red-100 p-3 flex items-center justify-between mb-2">
          ...
          <button
            ...
            className={`w-full font-bold py-4 rounded-xl shadow-lg active:scale-[0.98] transition-transform uppercase tracking-wider ${DuLieu.TrangThai === 2 && timeLeft < 60
              ...
              } text-white ${timeLeft === 0 && DuLieu.TrangThai === 2 ? 'grayscale cursor-not-allowed' : ''}`}
          >
            {timeLeft === 0 && DuLieu.TrangThai === 2 ? 'Đã hết hạn thanh toán' : 'Đặt hàng'}
          </button>
        </footer>

        {/* MODAL CHỌN MÃ GIẢM GIÁ */}
        <div className={`fixed inset-0 z-50 flex items-end transition-all duration-300 ${isModalOpen ? 'visible' : 'invisible'}`}>
          <div onClick={toggleModal} className={`absolute inset-0 bg-black/40 transition-opacity duration-300 ${isModalOpen ? 'opacity-100' : 'opacity-0'}`}></div>
          <div className={`relative bg-white w-full rounded-t-3xl p-6 max-h-[80vh] transform transition-transform duration-300 ease-in-out ${isModalOpen ? 'translate-y-0' : 'translate-y-full'}`}>
            ...
            <div className="flex space-x-2 mb-6">
              <input type="text" placeholder="Nhập mã voucher tại đây" className="flex-1 bg-gray-100 px-4 py-3 rounded-xl text-sm focus:outline-none ring-1 ring-gray-200 focus:ring-blue-400 transition-shadow" />
              <button className="bg-gray-200 text-gray-500 px-6 py-3 rounded-xl font-bold text-sm hover:bg-gray-300 transition-colors">Áp dụng</button>
            </div>
              ...
              {loading ? (
                ...
                maGiamGia.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <p className="text-gray-400 text-sm px-10 leading-relaxed">Hiện tại chưa có mã giảm giá nào khả dụng.</p>
                  </div>
                )
              )}
            </div>
